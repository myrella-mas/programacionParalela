...
int main(int argc, char * argv[]) {
    ...
    B = new long *[rank_size]; // Reservamos tantas filas como procesos haya
    x = new long [rank_size]; // El vector sera del mismo tamanio que el numero
    // de procesadores

    // Solo el proceso 0 ejecuta el siguiente bloque
    if (mi_rank == 0) {
        B[0] = new long [rank_size * rank_size];
        for (unsigned int i = 1; i < rank_size; i++) {
            B[i] = B[i - 1] + rank_size;
        }
        // Reservamos especio para el resultado
        y = new long [rank_size];

        // Rellenamos 'B' y 'x' con valores aleatorios
        srand(time(0));
        cout << "La matriz y el vector generados son " << endl;
        for (unsigned int i = 0; i < rank_size; i++) {
            for (unsigned int j = 0; j < rank_size; j++) {
                if (j == 0) cout << "[";
                B[i][j] = rand() % 1000;
                cout << B[i][j];
                if (j == rank_size - 1) cout << "]";
                else cout << "  ";
            }
            x[i] = rand() % 100;
            cout << "\t  [" << x[i] << "]" << endl;
    ...
    MPI_Scatter(B[0], // Matriz que vamos a compartir
            rank_size, // Numero de columnas a compartir
    ...
    MPI_Bcast(x, // Dato a compartir
            rank_size, // Numero de elementos que se van a enviar y recibir
